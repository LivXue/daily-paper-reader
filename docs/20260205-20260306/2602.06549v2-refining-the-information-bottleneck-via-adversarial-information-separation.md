...
tldr: 针对材料科学等领域中实验数据噪声大且缺乏标注的问题，本文提出对抗性信息分离框架（AdverISF）。该框架通过自监督对抗机制强制任务相关特征与噪声表示统计独立，并利用多层分离架构回收被误判为噪声的有用信息。实验证明，AdverISF在数据稀缺和真实材料设计任务中均优于现有方法，显著提升了模型的泛化能力。
selection_source: fresh_fetch
motivation: 传统正则化和对抗方法在缺乏显式分离标签的情况下，难以从高噪声的有限数据中精准提取任务相关特征。
method: 提出AdverISF框架，结合自监督对抗机制与多层回收架构，实现特征与噪声的深度分离与精细提取。
result: 在数据稀缺场景及真实材料设计任务中，该方法均取得了优于当前最先进技术的泛化性能。
conclusion: AdverISF通过无需显式监督的信息瓶颈优化，为高噪声、少样本领域的特征提取提供了有效解决方案。
---

## 摘要
从有限数据中进行泛化对于材料科学等领域的模型尤为关键，在这些领域中，实验数据集中的任务相关特征往往与测量噪声和实验伪影严重混淆。标准的正则化技术无法精确地将有意义的特征与噪声分离，而现有的对抗性自适应方法则受限于其对显式分离标签的依赖。为了应对这一挑战，我们提出了对抗性信息分离框架（AdverISF），该框架在不需要显式监督的情况下将任务相关特征从噪声中分离出来。AdverISF 引入了一种自监督对抗机制，以强制任务相关特征与噪声表示之间保持统计独立性。它进一步采用了一种多层分离架构，在特征层级中逐步回收噪声信息，以恢复被误当作噪声丢弃的特征，从而实现更细粒度的特征提取。广泛的实验表明，AdverISF 在数据稀缺场景下优于最先进的方法。此外，在真实世界材料设计任务上的评估显示，它实现了卓越的泛化性能。
